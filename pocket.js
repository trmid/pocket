...
(function (Pocket_1) {
    ...
    class SubPocket {
        ...
        search(radius, center, scale) {
            var found = new Array();
            const pos = scale ? Pocket.Tools.mul(this.position, scale) : this.position;
            const diff = Pocket.Tools.sub(pos, center);
            const dist = Pocket.Tools.mag(diff);
            if (dist - radius < this.radius) {
                for (let i = 0; i < this.particles.length; i++) {
                    const p = this.particles[i];
                    const p_pos = scale ? Pocket.Tools.mul(p, scale) : p;
                ...
                for (let i = 0; i < this.pockets.length; i++) {
                    found = found.concat(this.pockets[i].search(radius, center, scale));
                }
            }
            return found;
        }
    ...
    class Pocket {
        ...
        search(radius, center, scale) {
            if (!center.z)
                center.z = 0;
            if (this.root) {
                return this.root.search(radius, center, scale);
            }
            else {
                return new Array();
            }
